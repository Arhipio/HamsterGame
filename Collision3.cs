...
public class Collision3 : MonoBehaviour {
	...
	void OnTriggerEnter(Collider other){
		if (other.GetComponent<powerupactive> ().powerupnumber == 1&&(other.GetComponent<Name> ().name.Equals ("Player4"))&&timer>15) {
		...
		else if ((other.GetComponent<Name> ().name.Equals ("Player4"))&&timer>15) {
			ovx = other.GetComponent<Rigidbody> ().velocity.x;
			ovy = other.GetComponent<Rigidbody> ().velocity.y;
			ovz = other.GetComponent<Rigidbody> ().velocity.z;
